...
/*
    if (!this->parse_modbus_byte_(data[pos])) {
        this->clear_rx_buffer_(LOG_STR("parse_tcp failed"), true);
      }
...
void Modbus::loop() {
   ...
   if (!connected_ && this->tcp_or_rtu_) {
...
if(!this->tcp_or_rtu_) {
      ...
      if (!this->parse_modbus_byte_(buf[i])) {
        this->clear_rx_buffer_(LOG_STR("parse_rtu failed"), true);
      }
      this->last_modbus_byte_ = millis();
    }
  }
...
void Modbus::send_next_frame_() {
  ...
  if (this->flow_control_pin_ != nullptr) {
    ...
    if (this->tcp_or_rtu_) {
   this->send_message(frame.data.get());
     } else {
...
void Modbus::dump_config() {
  ...
  ESP_LOGCONFIG(TAG,
                     ...
                     "  Long Rx Buffer Delay: %d ms\n", 
                        ...
                        this->turnaround_delay_ms_, this->frame_delay_ms_,
                        this->long_rx_buffer_delay_ms_);
  } else {
    ESP_LOGCONFIG(TAG,
                "Modbus_RTU:\n"
